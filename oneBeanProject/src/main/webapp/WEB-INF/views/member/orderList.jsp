...
<link href="<%=request.getContextPath() %>/css/member/orderList.css" rel="stylesheet" />
...
<script type="text/javascript">
	function searchOrder() {
		...
		$.ajax({
			url:"<%=request.getContextPath() %>/Purchase/searchOrder.do",
			...
			success:function(data){
				...
				if (list != 0 ){
					...
					$.each(data.list , function(i){ 
						tr += '<tr><td>' + data.list[i].payDate + '</td>';
						tr += '<td>' + data.list[i].proName + '</td>';
						tr += '<td>' + data.list[i].proContents + '</td>';
						tr += '<td>' + data.list[i].payOneCount + '</td>';
						tr += '<td>' + data.list[i].payOnePrice + '</td></tr>';
						});
...
<section id="section">
	<img class="title_image" src="<%=request.getContextPath() %>/images/order.png">
	<div id="backbody">
		<div id="frame">
