...
<link href="<%=request.getContextPath() %>/css/manager/memberList.css" rel="stylesheet" />
	...
	<img class="title_image" src="<%=request.getContextPath()%>/images/게시판.png">
          ...
          <c:if test="${member != null}">
	          <c:if test="${memberGrade eq 0 }">
		          <c:forEach var="listAll" items="${listAll}">
			          ...
			          <tr onclick="location.href='<%=request.getContextPath()%>/Question/view.do?qidx=${listAll.qidx}'" style="cursor: pointer;">
	          ...
	          <c:if test="${memberGrade ne 0 }">
	          	<c:forEach var="list2" items="${list}">
			          ...
			          <tr onclick="location.href='<%=request.getContextPath()%>/Question/view.do?qidx=${list2.qidx}'" style="cursor: pointer;">
        ...
        <c:if test="${member != null}">
	        <div style="padding-left:85%;">
	        	<button class="btn btn-light" onclick="location.href='<%=request.getContextPath() %>/Question/write.do'">문의하기</button>
	        	<c:if test="${memberGrade eq 0 }">
	        	<button class="btn btn-light" onclick="location.href='<%=request.getContextPath() %>/Question/deleteBoard.do'">삭제게시판</button>
	        	</c:if>
	        </div>
        </c:if>
        ...
        <c:if test="${memberGrade eq 0 }">
        	<c:if test="${paging.startPage != 1 }">
        		<a href="<%=request.getContextPath() %>/Question/list.do?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
        	</c:if>
        	<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
        		<c:choose>
        			...
        			<c:when test="${p != paging.nowPage }">
        				<a href="<%=request.getContextPath() %>/Question/list.do?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
        			</c:when>
        		</c:choose>
        	</c:forEach>
        	<c:if test="${paging.endPage != paging.lastPage }">
        		<a href="<%=request.getContextPath() %>/Question/list.do?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage }">&gt;</a>
        	</c:if><br><br>
        </c:if>
       	<c:if test="${memberGrade ne 0 }">
       		<c:if test="${pagingMember.startPage != 1 }">
        		<a href="<%=request.getContextPath() %>/Question/list.do?nowPage=${pagingMember.startPage - 1 }&cntPerPage=${pagingMember.cntPerPage}">&lt;</a>
        	</c:if>
        	<c:forEach begin="${pagingMember.startPage }" end="${pagingMember.endPage }" var="p">
        		<c:choose>
        			...
        			<c:when test="${p != pagingMember.nowPage }">
        				<a href="<%=request.getContextPath() %>/Question/list.do?nowPage=${p }&cntPerPage=${pagingMember.cntPerPage}">${p }</a>
        			</c:when>
        		</c:choose>
        	</c:forEach>
        	<c:if test="${pagingMember.endPage != pagingMember.lastPage }">
        		<a href="<%=request.getContextPath() %>/Question/list.do?nowPage=${pagingMember.endPage+1 }&cntPerPage=${pagingMember.cntPerPage }">&gt;</a>
        	</c:if><br><br>
       	</c:if>
        </div>
