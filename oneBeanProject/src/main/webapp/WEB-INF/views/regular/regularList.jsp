...
<section class="py-5">
					...
					<div class="card h-100">
						...
						<div class="card-body p-4">
							...
							<c:if test="${member.memberGrade == 0}">
								<button class="btn btn-light" onclick="location.href='<%=request.getContextPath() %>/Manager/regModify.do?regIdx=${list.regIdx}'">수정하기</button>
	...
	<div style="display:block; text-align:center;">
        	<c:if test="${paging.startPage != 1 }">
        		<a href="<%=request.getContextPath() %>/Regular/regularList.do?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
        	</c:if>
        	<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
        		<c:choose>
        			...
        			<c:when test="${p != paging.nowPage }">
        				<a href="<%=request.getContextPath() %>/Regular/regularList.do?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
        			</c:when>
        		</c:choose>
        	</c:forEach>
        	<c:if test="${paging.endPage != paging.lastPage }">
        		<a href="<%=request.getContextPath() %>/Regular/regularList.do?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage }">&gt;</a>
        	</c:if><br><br>
